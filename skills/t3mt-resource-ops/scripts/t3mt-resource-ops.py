#!/usr/bin/env python3
"""Resource-focused assistant helper."""
...
def main() -> None:
    ...
    mapping = {
        ...
        "action": "run-resource-action",
        "search-sources": "search-sources",
        "catalog-sources": "catalog-sources",
    }
    target = mapping.get(command)
    if not target:
        print("Usage: t3mt-resource-ops.py <search|catalog|detail|actions|action|search-sources|catalog-sources> [key=value ...]", file=sys.stderr)
        raise SystemExit(1)
    raise SystemExit(
        subprocess.run(
